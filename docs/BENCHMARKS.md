...
| our t0-fast WebGPU, Q8_0-resident (tiled GEMM) | 108.9 | 197.3 | **33.6** | 18.1 | 0.20% | — | — |
| our t0-fast WebGPU, Q4_0-resident | 58.6 | 78.9 | **29.4** | 15.3 | 3.84% | — | — |
| our t0-fast WebGPU, F32-resident | 108.9 | 194.5 | 84.4 | 71.8 | 0.20% | — | — |
...
**Both t0-fast quantized rows now beat their single-call warm number** (33.6ms and 29.4ms vs their 64.0ms) — the first time any engine in this repo has beaten the ORT WebGPU baseline on single-call latency. Batch-24 is closer but not there yet (18.1/15.3ms/signal vs their 8.87ms/signal, both still ahead of every Burn row's 34.0-34.2ms/signal though). `t0-fast` is a separate engine (`crates/t0-fast`, raw `wgpu`, no Burn at inference — see `docs/runs/2026-09-20-perf.md`'s Phase A-D). The Q8_0/Q4_0-resident rows load the GGUF's block bytes straight into GPU buffers with no F32-expansion round trip (`GpuModel::load_from_gguf_bytes`) and use a tiled shared-memory GEMM kernel for M>=32 (Q8_0 and F32; Q4_0 measured worse tiled and stays on the naive per-thread kernel, see that doc). Their exact-match GIFT-Eval-subset CRPS/MASE (footnote below) confirms these are numerically the same forecasts as the existing "ours Q8_0"/"ours Q4_0" rows, not a different quantization — the official-protocol (8192-context) cell is still blank for `t0-fast` because that protocol needs autoregressive rollout, not implemented by any backend in this repo. Native numbers (single/batch-24 ms, M2, Metal): F32 96.9/41.2, Q8_0 101.1/41.8 (tiled), Q4_0 70.9/35.9 (naive) — every row is *slower* natively than in the browser for reasons not fully explained this session (see `docs/runs/2026-09-20-perf.md`'s open question). GPU memory (weights + pool working set after cold+11 single+1 batch-24 calls, browser): Q8_0 396.9MB, Q4_0 346.6MB.
...
**Quotable paragraph.** On the same M2, same headless Chromium build, same `us_births` window (context 512, horizon 32): their official `t0-alpha-onnx-int8` export runs on onnxruntime-web's WebGPU EP at 64.0ms warm, 8.87ms/signal batched. Fused-Burn (`burn-wgpu` `fusion`) is still slower on both axes (169.3-169.8ms warm, 34.0-34.2ms/signal batch). `t0-fast` — a second engine with no Burn at inference, raw `wgpu`/WGSL kernels, GGUF block bytes kept resident (no F32 expansion), one command encoder per forecast — changes the picture: its Q8_0-resident build is **33.6ms warm, nearly 2x faster than their ONNX export**, and Q4_0-resident is **29.4ms, over 2x faster**; both are the first rows in this repo's history to beat the ORT baseline on single-call latency. Batch-24 narrows the historical 4-5x deficit to about 2x (18.1/15.3ms/signal vs their 8.87ms/signal) — closer than fused-Burn ever got (34.0-34.2ms/signal) but still behind, because only two of `t0-fast`'s three weight residencies (F32, Q8_0) got a tiled shared-memory GEMM this session; Q4_0's naive kernel was already faster than its own tiled variant, so it wasn't switched. Numerical fidelity is unaffected: `t0-fast`'s Q8_0/Q4_0-resident forecasts match Burn's own quantized path to kernel-level float rounding (~1-2e-6 max-abs, see `docs/runs/2026-09-20-perf.md`) and reproduce Burn's GIFT-Eval-subset CRPS/MASE bit-for-bit at the aggregate level (footnote above) — this is a latency win with no accuracy cost, not a different point on a speed/accuracy tradeoff curve. What's still open: the official 8192-context protocol's CRPS/MASE cells are blank for `t0-fast` (blocked on autoregressive rollout, no backend in this repo has it); the native numbers are, oddly, all slower than the browser numbers for the same engine and quantization (Q8_0 native 101.1ms vs browser 33.6ms) — not yet explained, flagged as a follow-up rather than chased further; and batch-24 still needs either a Q4_0 tiled kernel that actually helps or a different batching strategy to close the remaining ~2x gap to 8.87ms/signal.
