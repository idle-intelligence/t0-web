    ...
    for (i, layer) in model.layers.iter().enumerate() {
        ...
        let attn_out = match layer.ty {
            LayerType::Time => {
                ...
                rmsnorm_qk(engine, &mut encoder, &lk("qnorm"), &qkv, &layer.q_norm, rows, heads, head_dim, 0, 3 * embed);
                rmsnorm_qk(engine, &mut encoder, &lk("knorm"), &qkv, &layer.k_norm, rows, heads, head_dim, embed, 3 * embed);
                rope(engine, &mut encoder, &lk("ropeq"), &qkv, &cos_buf, &sin_buf, &scale_buf, rows, heads, head_dim, 0, 3 * embed, p);
                rope(engine, &mut encoder, &lk("ropek"), &qkv, &cos_buf, &sin_buf, &scale_inv_buf, rows, heads, head_dim, embed, 3 * embed, p);
