// Tiled shared-memory GEMM, Q8_0-resident `w`, 2x2 register blocking (see
// linear_tiled.wgsl for the blocking scheme and linear_q8.wgsl for the
// block-decode math): the B tile is dequantized once per 32x16 block
// element (not per output thread), and each thread reuses both operand
// registers across a 2x2 output sub-tile.
const TM: u32 = 32u;
const TN: u32 = 32u;
const TK: u32 = 16u;
